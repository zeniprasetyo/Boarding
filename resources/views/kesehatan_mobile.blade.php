...
<head>
    ...
    <title>Kesehatan Santri | Boarding Admin</title>
...
<body>
    <!-- Header -->
    <div class="header">
        <h1><i class="fas fa-heartbeat"></i> Kesehatan Harian Santri</h1>
        <p class="subtitle">Pantau kondisi kesehatan santri dengan sistematis</p>
    ...
    <main class="container">
        ...
        @if(auth()->user()->hasRole('musyrif') && !auth()->user()->hasRole('admin'))
            <div class="role-alert warning fade-in">
                ...
                <div>
                    ...
                    <small>Anda hanya dapat mengisi data kesehatan untuk halaqah Anda</small>
                </div>
            </div>
        @endif
        ...
        <div class="card fade-in">
            <div class="card-content">
                <h5 class="mb-3"><i class="fas fa-filter"></i> Filter Kesehatan</h5>
        ...
        <div class="info-box fade-in" id="infoBox">
            ...
            <div class="info-box-content">
                <div class="info-box-title">TANGGAL PENCATATAN</div>
        ...
        <div class="empty-state fade-in" id="emptyState">
            ...
            <h5>Belum ada data kesehatan</h5>
        ...
        <div class="action-buttons fade-in" id="actionButtons" style="display: none;">
            <button type="button" class="action-btn set-all-btn" onclick="setAllStatus('sehat')">
                <i class="fas fa-check-circle text-success"></i> Semua Sehat
            </button>
            <button type="button" class="action-btn set-all-btn" onclick="setAllStatus('sakit')">
                <i class="fas fa-hospital text-danger"></i> Semua Sakit
            </button>
            <button type="button" id="saveKesehatanBtn" class="action-btn save-btn">
    ...
    <nav class="mobile-nav">
        <div class="mobile-nav-container">
            ...
            <a href="/kesehatan" class="mobile-nav-item active">
    ...
    <script>
        $(document).ready(function() {
            ...
            const statusMapping = {
                ...
                'rumah sakit': { 
                    text: 'Rumah Sakit', 
                    class: 'status-rumah-sakit', 
                    icon: 'hospital',
                    color: 'dark'
                },
                'rawat jalan': { 
                    text: 'Rawat Jalan', 
                    class: 'status-rawat-jalan', 
                    icon: 'stethoscope',
                    color: 'info'
                }
            };
            ...
            function loadKesehatanData() {
                ...
                $('#santriContainer').html(`
                    <div class="text-center py-4">
                        ...
                        <p class="mt-2">Memuat data kesehatan...</p>
                    </div>
                `);
                
                // AJAX request
                $.ajax({
                    url: "/kesehatan/data-mobile", 
                    method: 'GET',
                    data: {
                    ...
                    success: function(response) {
                        if (response.success && response.data.length > 0) {
                            displayKesehatanData(response.data, tanggal, halaqahId);
                        } else {
                            showEmptyState();
                            $('#infoBox').hide();
                            $('#actionButtons').hide();
                    ...
                    error: function(xhr) {
                        console.error('Error loading kesehatan data:', xhr);
                        
                        let errorMessage = 'Terjadi kesalahan saat memuat data kesehatan.';
                        if (xhr.status === 403) {
            ...
            // Display data ke UI
            function displayKesehatanData(data, tanggal, halaqahId) {
            ...
            function generateSantriCard(santri, index) {
                const statusBadge = getStatusBadge(santri.status);
                
                return `
                    <div class="santri-card fade-in" style="animation-delay: ${index * 0.05}s">
                        <div class="santri-header">
                            <div class="santri-avatar">
                                ${santri.name ? santri.name.charAt(0).toUpperCase() : 'S'}
                            </div>
                            <div class="santri-info">
                                <div class="santri-name">${santri.name || 'Santri'}</div>
                                <div class="santri-id">#${santri.id}</div>
                            </div>
                        </div>
                        
                        <div class="kesehatan-status">
                            <div class="kesehatan-select-container">
                                <div class="kesehatan-label">
                                    <i class="fas fa-heartbeat text-danger"></i> Status Kesehatan
                                </div>
                                <select class="kesehatan-select status-select" 
                                        data-santri-id="${santri.id}">
                                    ${getStatusOptions(santri.status)}
                                </select>
                                <div class="status-badge-container" id="badge-${santri.id}">
                                    ${statusBadge}
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="keterangan-box">
                            <div class="keterangan-label">
                                <i class="fas fa-sticky-note text-muted"></i> Keterangan
                            </div>
                            <input type="text" 
                                   class="keterangan-input" 
                                   data-santri-id="${santri.id}"
                                   value="${santri.keterangan || ''}"
                                   placeholder="Misal: Flu, Pusing, Demam, dll.">
                        </div>
                    </div>
                `;
            }
            ...
            function initializeEventListeners() {
                ...
                $('.status-select').off('change').on('change', function() {
                    ...
                    const badgeContainer = $(`#badge-${santriId}`);
                    
                    if (status) {
                        const badge = getStatusBadge(status);
                        badgeContainer.html(badge);
                        
                        // Auto-focus keterangan jika status tertentu
                        if (status === 'sakit' || status === 'rumah sakit' || status === 'rawat jalan') {
                            const card = $(this).closest('.santri-card');
                            card.find('.keterangan-input').focus();
                        }
                    } else {
                        badgeContainer.html('');
                    }
                ...
                // Keterangan input
                $('.keterangan-input').off('input').on('input', function() {
                    const santriId = $(this).data('santri-id');
                    // Bisa tambahkan logika validasi atau auto-save jika diperlukan
                });
                
                // Save button
                $('#saveKesehatanBtn').off('click').on('click', function() {
                    saveKesehatanData();
                });
            }
            ...
            window.setAllStatus = function(status) {
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil!',
                    text: `Semua status kesehatan diubah menjadi ${statusMapping[status]?.text || 'Sehat'}`,
                    timer: 1500,
            ...
            Swal.fire({
                ...
                text: 'Tidak ada data kesehatan yang diubah',
                confirmButtonColor: '#1a5fb4'
            });
            return;
        }
        ...
        // Show loading
        const saveBtn = $('#saveKesehatanBtn');
        const originalText = saveBtn.html();
        saveBtn.html('<i class="fas fa-spinner fa-spin"></i> Menyimpan...');
        ...
        $.ajax({
            ...
            error: function(xhr) {
                console.error('Error saving kesehatan data:', xhr);
                
                let errorMessage = 'Terjadi kesalahan saat menyimpan data.';
                if (xhr.status === 403) {
                    errorMessage = 'Anda tidak memiliki izin untuk menyimpan data ini.';
    ...
    // Auto-refresh data setiap 30 detik jika sedang aktif
    let refreshInterval;
    ...
    // Start auto-refresh saat halaman aktif
    $(window).on('focus', startAutoRefresh);
    $(window).on('blur', function() {
        if (refreshInterval) clearInterval(refviewerInterval);
    });
